...
if __name__ == "__main__":
    ...
    search_space = {
        ...
        'ablation': tune.grid_search([1,2,3]),
        'init_pred': tune.sample_from(lambda config: 1 if (config['ablation'] == 1) else configs_4b[config_id(config)]['init_pred']),
        'p_left': tune.sample_from(lambda config: 0.0 if (config['ablation'] < 3) else configs_4b[config_id(config)]['p_left']),
        'p_right': 1.0,
